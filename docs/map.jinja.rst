...
We will see a quick setup to configure the ``firefox`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``firefox`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``firefox`` formulas under ``/srv/salt/firefox/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/firefox/parameters/dns:domain/
    ln -s dns:domain /srv/salt/firefox/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/firefox/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/firefox/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``firefox`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``firefox`` formulas under ``/srv/salt/firefox/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/firefox/parameters/roles
...
We create a configuration for the role ``firefox/server`` in ``/srv/salt/firefox/parameters/roles/firefox/server.yaml``:
...
We create another configuration for the role ``firefox/client`` in ``/srv/salt/firefox/parameters/roles/firefox/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles firefox/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles firefox/client
...
.. code-block:: console
    ...
    mv /srv/salt/firefox/parameters/dns:domain/  '/srv/salt/firefox/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-firefox-as-json:
      test.nop:
        - name: {{ firefox | json }}
...
Here is an example based on `template-formula/firefox/config/file.sls`_:
...
This ``sls`` file expose a ``firefox`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ firefox['winner'] }}
...
.. _template-formula/firefox/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/firefox/config/file.sls
